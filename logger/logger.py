...
class Logger:
    ...
    handler1 = RotatingFileHandler(LoggerConfig.LOGS_FILE_NAME, maxBytes=LoggerConfig.MAX_BYTES,
                                   backupCount=LoggerConfig.BACKUP_COUNT)
    handler2 = logging.StreamHandler(sys.stdout)
    __formatter = logging.Formatter(LoggerConfig.FORMAT)
    handler1.setFormatter(__formatter)
    handler2.setFormatter(__formatter)
